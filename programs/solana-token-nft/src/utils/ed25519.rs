...
pub fn check_ed25519_data(data: &[u8], pubkey: &[u8], msg: &[u8], sig: &[u8]) -> Result<()> {
    ...
    {
        return Err(SigError::SigVerificationFailed.into());
    }
    msg!("Debug: data_pubkey = {:?}", data_pubkey );
    msg!("Debug: data_msg = {:?}", data_msg );
    msg!("Debug: data_sig = {:?}", data_sig );
    // Arguments
